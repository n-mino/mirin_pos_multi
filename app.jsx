    ...
    <div style={{ background: COLORS.paper, border: `1.5px solid ${COLORS.line}`, borderRadius: 10, padding: 20 }}>
      ...
      <GuideSection title="② 売上管理">
        <GuideItem label="売上履歴">
          「本日のみ」「すべて」、または日付を指定して過去の会計記録を一覧表示できます。一覧上部に該当件数・合計金額を表示します。行をタップすると明細(注文内容)を確認できます。呼込み/同伴の担当者がいて、小計が一定額を超えた会計には、従業員マスタで設定した率に応じた「売上バック」が自動計算され列に表示されます(条件:小計30,000円超、または5人以上で小計50,000円超。両方満たす場合は高い方の率を採用)。画面右上の「CSVダウンロード」で、表示中の絞り込み条件のままCSVファイルとして書き出せます。
        </GuideItem>
      ...
      <GuideSection title="④ マスタ設定">
        ...
        <GuideItem label="バックアップの復元(全件復元/期間指定)">
          復元前に「全件復元」「期間指定」のどちらかを選びます。どちらも商品・座席・従業員マスタ・設定などは選択したファイルの内容にまるごと置き換わります(現在使用中の座席は復元対象外)。「全件復元」は売上履歴・勤怠・入出金もすべて選択したファイルの内容で復元しますが、「期間指定」はこの3つだけ、選択したファイルに含まれる直近1・3・6ヶ月のいずれかの期間分のみを復元します(容量が多くなってきた際に、事前にバックアップを書き出してから「全データ削除」→「期間指定」で復元すると、マスタは保ったまま直近データだけ残せます)。いずれもファイルを選択すると、実際に削除・復元される件数が確認画面に表示されるので、内容を確認してから実行してください。商品・座席・設定・勤怠の変更はSupabase経由で他の端末にも即座に反映されます(従業員マスタはこの復元では反映されません。売上履歴・入出金はこの端末のみです)。
        </GuideItem>
...
function SettingsScreen({ data, onBack, onUpdateProducts, onUpdateSeatCount, onUpdateSeatName, onUpdateRates, onUpdateSeatToneThresholds, onUpdatePayroll, onImportData, onImportDataPeriod, onDeleteAllData, onUpdateSecurity, onResetSecurity, showToast, myEmployee, onLogout }) {
  ...
  // 取得し直せるボタンを用意する(従業員マスタタブを開いた際にも自動で1回実行する)。
  ...
  const resetEmployeePassword = async (employee) => {
    ...
    // functions.invoke()のセッション自動添付に任せると、ログイン中でも
    // Authorizationヘッダーが空のまま送られ、Edge Function側でgetUser(jwt)が
    // AuthSessionMissingErrorになる事象が確認されたため、現在のアクセストークンを
    // 明示的に取得してヘッダーに付与する。
    const { data: sessionData } = await window.supabaseClient.auth.getSession();
    const accessToken = sessionData?.session?.access_token;
    if (!accessToken) {
      setResettingPasswordId(null);
      showToast("パスワード再発行に失敗しました: ログインセッションが確認できません。再ログインしてからお試しください。");
      return;
    }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      <div style={{ display: "flex", gap: 6, padding: "12px 20px", borderBottom: `1px solid ${COLORS.line}`, background: COLORS.paper, overflowX: "auto" }}>
        {[{ id: "products", label: "商品管理" }, { id: "seats", label: "座席設定" }, { id: "rates", label: "税・サービス料" }, { id: "staff", label: "従業員マスタ" }, { id: "password", label: "パスワード設定" }, { id: "data", label: "データ管理" }, { id: "help", label: "使用方法" }].map((t) => (
          <button
            key={t.id}
        ...
        {tab === "staff" && (
          <div style={{ display: "flex", flexDirection: isNarrow ? "column" : "row", gap: 20 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {!isNarrow && (
                <div style={{ visibility: "hidden", marginBottom: 16 }} aria-hidden="true">
                  <TicketButton variant="primary" icon={Plus} style={{ pointerEvents: "none" }}>従業員を追加</TicketButton>
                </div>
              )}
      ...
      {pendingRestoreConfirm && pendingRestoreConfirm.mode === "full" && (
        <ConfirmModal
          title="全件復元"
          message="現在の商品・座席・売上履歴・設定などすべてのデータを削除し、選択したファイルの内容で復元します。商品・座席・設定・勤怠はSupabase経由で他の端末(スタッフのスマホ等)にも即座に反映されます(売上履歴・入出金・パスワード設定はこの端末のみ)。この操作は取り消せません。よろしいですか?"
          confirmLabel="復元する"
      ...
      {pendingRestoreConfirm && pendingRestoreConfirm.mode === "period" && (
        <ConfirmModal
          ...
          message={
            <>
              現在の商品・座席・設定などのマスタ、および売上履歴・勤怠・入出金をすべて削除し、選択したファイルの内容で復元します(現在使用中の座席は復元対象外です)。ただし売上履歴・勤怠・入出金は、直近{pendingRestoreConfirm.months}ヶ月分のみを復元します。商品・座席・設定・勤怠はSupabase経由で他の端末(スタッフのスマホ等)にも即座に反映されます(売上履歴・入出金はこの端末のみ)。この操作は取り消せません。
              <br /><br />
              復元されるデータ件数
              <br />
      ...
      {pendingDeleteAllConfirm && (
        <ConfirmModal
          title="全データ削除"
          message="商品・座席・売上履歴・設定などすべてのデータを削除します。商品・座席・設定・勤怠はSupabase経由で他の端末(スタッフのスマホ等)にも即座に反映されます(売上履歴・入出金・パスワード設定はこの端末のみ)。本当に削除しますか？"
          confirmLabel="削除する"
      ...
      {deletingEmployee && (() => {
        ...
        return (
          <ConfirmModal
            title={hasShifts ? "退職済みにしますか？" : "従業員を削除しますか？"}
            message={
              hasShifts
